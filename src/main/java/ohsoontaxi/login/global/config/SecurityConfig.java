...
import ohsoontaxi.login.global.security.filter.CustomAuthenticationEntryPoint;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        http
                ...
                .and()
                .authorizeHttpRequests() // 인증절차에 대한 설정을 진행
                .requestMatchers("/api/v1/credentials/oauth/valid/register","/api/v1/credentials/register",
                        "/api/v1/credentials/login").permitAll() // 설정된 url은 인증되지 않더라도 누구든 접근 가능
